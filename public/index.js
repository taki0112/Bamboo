        ...
        d3.select(_id).append("svg")
                ...
                .each(function () {
                  d3.select(this).on("click", function (d) {
                    console.log(cur_univ +" , " + d.text)
                    run_word2vec(cur_univ,d.text)
                  });
                });
    }
...
function show_vector(){
    ...
    for(var i = 0; i < elem.length-1; i++){
        ...
        var k = i==0? 1 : i;
        k++;
        score =  Number(parseFloat(score)*50)
        var input = Number(score)*(Math.log(12-k)/Math.log(5))
...
function show_ranking(){
  ...
  for(var i = 0; i < tf_idf.length; i++){
      ...
      var k = i==0? 1 : i;
      k++;
      score =  Number(parseFloat(score)/30)
      var input = Number(score)*(Math.log(12-k)/Math.log(5))
      var temp = {"text":keyword,"size":input}
      result.push(temp)
